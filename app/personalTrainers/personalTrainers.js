...
async function aoCarregarPagina() {
    ...
    document.querySelector("#btn-imprimir").onclick = imprimirListaDePersonalTrainers;
...
async function buscarPersonalTrainers() {
    try {
        ...
        listaDePersonalTrainers = await servicos.buscarDados(token, nome, status, cadastro);

        document.querySelector("#lista-personal-trainers").innerHTML = "";

        if (listaDePersonalTrainers.length > 0) {
            listaDePersonalTrainers.forEach(personalTrainer => {
...
function imprimirListaDePersonalTrainers() {
    const linhas = [];
    
    linhas.push([{ text: 'Nome', style: 'tableHeader'}, { text: 'E-mail', style: 'tableHeader'}, { text: 'Cadastro', style: 'tableHeader'}, { text: 'Status', style: 'tableHeader'}]);

    listaDePersonalTrainers.forEach(personal => {
        const colunas = [];

        Object.entries(personal).forEach(([nome, valor]) => {
            ...
            if (nome == "cadastroConfirmado" && valor == 0) {
                valor = `Pendente`

            } else if (nome == "cadastroConfirmado" && valor == 1){
                valor = `Confirmado`

            } else if (nome == "cadastroConfirmado" && !valor){
                valor = `Recusado`
            }
    ...
    var docDefinition = {
        ...
        content: [
            ...
            {
                stack: [
                    { text: 'Lista de Personal Trainers:', style: 'subheader' },
                    {
                        style: 'tableExample',
                        table: {
                            headerRows: 1,
