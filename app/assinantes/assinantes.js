...
async function aoCarregarPagina() {
    ...
    document.querySelector("#btn-imprimir").onclick = imprimirListaDeAssinantes;
...
async function buscarAssinantes() {
    ...
    try {
        ...
        listaDeAssinantes = await servicos.buscarDados(token, nome, status);

        document.querySelector("#lista-assinantes").innerHTML = "";

        if (listaDeAssinantes.length > 0) {
            listaDeAssinantes.forEach(assinante => {
...
function imprimirListaDeAssinantes() {
    const linhas = [];
    
    linhas.push([{ text: 'Nome', style: 'tableHeader'}, { text: 'E-mail', style: 'tableHeader'}, { text: 'Status', style: 'tableHeader'}]);

    listaDeAssinantes.forEach(assinante => {
        const colunas = [];

        Object.entries(assinante).forEach(([nome, valor]) => {
            if(nome == "idAssinante"){
                return;
            }
    ...
    var docDefinition = {
        ...
        content: [
            ...
            {
                stack: [
                    { text: 'Lista de Assinantes:', style: 'subheader' },
                    {
                        style: 'tableExample',
                        table: {
                            headerRows: 1,
