...
async function aoCarregarPagina() {
    ...
    document.querySelector("#btn-imprimir").onclick = imprimirListaDeNutricionistas;
...
async function buscarNutricionistas() {
    try {
        ...
        listaDeNutricionistas = await servicos.buscarDados(token,nome, status, cadastro);

        document.querySelector("#lista-nutricionistas").innerHTML = "";

        if (listaDeNutricionistas.length > 0) {
            listaDeNutricionistas.forEach(nutricionista => {
...
function imprimirListaDeNutricionistas() {
    const linhas = [];
    ...
    listaDeNutricionistas.forEach(nutri => {
        const colunas = [];

        Object.entries(nutri).forEach(([nome, valor]) => {
    ...
    var docDefinition = {
        ...
        content: [
            ...
            {
                stack: [
                    { text: 'Lista de Nutricionistas:', style: 'subheader' },
                    {
                        style: 'tableExample',
                        table: {
                            headerRows: 1,
